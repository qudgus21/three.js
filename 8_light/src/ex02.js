...
export default function example() {
  ...
  //PointLight 한 지점에 있는 빛(세기, 거리)
  const light = new THREE.PointLight("tomato", 1, 100, 2);
  light.position.y = 3;
  scene.add(light);
  ...
  function draw() {
    ...
    light.position.x = Math.cos(time) * 5; //cos: x좌표
    light.position.z = Math.sin(time) * 5; //sin: z좌표
